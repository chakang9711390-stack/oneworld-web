...
export default async function AdminRolesPage() {
  ...
  return (
    <div className="grid gap-6">
      <SectionCard title="职业管理" description="当前先提供只读列表，后续再接编辑能力。" />
      <div className="overflow-hidden rounded-[26px] border border-[var(--line)] bg-[var(--panel)]">
        <div className="grid grid-cols-[1.3fr_1fr_120px] gap-4 border-b border-[var(--line)] px-5 py-4 text-xs uppercase tracking-[0.08em] text-[var(--text-faint)]">
          <span>职业</span>
          <span>行业</span>
          <span>工作流</span>
        </div>
        {items.map((item) => (
          <div key={item.id} className="grid grid-cols-[1.3fr_1fr_120px] gap-4 border-b border-[var(--line)] px-5 py-4 text-sm last:border-b-0">
            <div>
            ...
            <div className="text-[var(--text-soft)]">{item.workflowTemplates.length}</div>
          </div>
        ))}
      </div>
    </div>
